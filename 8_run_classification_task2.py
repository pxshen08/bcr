...
def parse_args():#原先是没有def的，直接由bash调用20240126
 ...
 parser.add_argument('--gene', default='VL',type=str, help="Gene type (VH, VL, JH, JL, isoH, isoL)")
...
# parser = argparse.ArgumentParser(description="Gene usage tasks")
# parser.add_argument("gene", type=str, help="Gene type (VH, VL, JH, JL, isoH, isoL)")
...
def load_data(gene, embedding):
    ...
    if embedding == 'esm2' or embedding == 'esm2_3B':
        ...
        X = torch.load("/home/mist/projects/Wang2023/data/BCR_embed/dataa/" + data_prefix + "_antiBERTy.pt",
                       map_location=torch.device('cpu')).numpy()
        print(X)
        ...
        X = pd.read_pickle("/home/mist/projects/Wang2023/data/BCR_embed/datap/" + data_prefix + "_" + embedding + ".pkl")
...
if args.random:
    ...
    is_random = ""
filename = "/home/mist/projects/Wang2023/data/result/" + args.embedding + "_" + args.gene + is_random + ".csv"
...
out_dir = "/home/mist/projects/Wang2023/data/model/" + args.embedding + "/"
