...
def load_data(gene, embedding):
    ...
    if embedding == 'esm2' or embedding == 'esm2_3B':
        ...
        X = torch.load("/home/mist/projects/Wang2023/data/BCR_embed/datai/" + data_prefix + "_"+str(embedding)+".pt", map_location=torch.device('cpu')).numpy()
        print(X)
...
# 将加载的数据移动到 GPU 20240131
#X = torch.tensor(X, dtype=torch.float32).to("cuda")
#y = torch.tensor(y, dtype=torch.long).to("cuda")
#y_groups = torch.tensor(y_groups, dtype=torch.long).to("cuda")
...
#     # get the cross validation score on the test
    ...
    y_groups_train, y_groups_test=y_groups[train_index], y_groups[test_index]#20240201
    ...
    splits = gss.split(X = X_train, groups = y_groups_train)
...
filename = out_dir + args.gene + ".csv"
