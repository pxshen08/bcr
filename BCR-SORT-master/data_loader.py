...
class DatasetBCRSORT(Dataset):
    def __init__(self, data_in, model):
        # encoding sequence
        if model !="antiBERTy":
        ...
        for variable in categorical_variable:
            data_in.loc[:, variable] = data_in.apply(
                lambda row: encode_categorical(row[variable], encoding_dict[variable]), axis=1)
...
def load_dataset(file_in, mode, model,test_ratio=None):
    ...
    # data_in = data_in[data_in['cdr3_aa'].str.strip() != 'nan']
    data_in = data_in.dropna(subset=['seq']).loc[~data_in['seq'].str.strip().eq('')]
    ...
    if test_ratio is None:
        return DatasetBCRSORT(data_in=data_in,model=model)
    else:
        data_train, data_val, data_test = split_train_test(data_in, test_ratio, seed=12345)
        TrainDatasetBCR = DatasetBCRSORT(data_in=data_train,model=model)
        ValDatasetBCR = DatasetBCRSORT(data_in=data_val,model=model)
        TestDatasetBCR = DatasetBCRSORT(data_in=data_test,model=model)
